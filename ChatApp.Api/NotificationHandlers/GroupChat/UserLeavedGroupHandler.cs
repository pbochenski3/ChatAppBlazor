...
namespace ChatApp.Api.NotificationHandlers.GroupChat
{
    public class UserLeavedGroupHandler : INotificationHandler<UserLeavedGroupNotification>
    {
        ...
        public async Task Handle(UserLeavedGroupNotification n, CancellationToken cancellationToken)
        {
            if (n.removed)
            {
                await _hubContext.Clients.User(n.UserId.ToString()).SendAsync("NotifyInfo", "Zostałes wyrzucony z czatu!");
            }
            else
            {
                await _hubContext.Clients.User(n.UserId.ToString()).SendAsync("NotifyInfo", "Opuściłeś czat!");
            }
            await _hubContext.Clients.User(n.UserId.ToString()).SendAsync("ChatReload", n.ChatId, true);
