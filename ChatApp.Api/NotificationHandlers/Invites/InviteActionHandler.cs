...
namespace ChatApp.Api.NotificationHandlers.Invites
{
    public class InviteActionHandler : INotificationHandler<InviteActionNotification>
    {
        ...
        {
            await Task.WhenAll(
                _hubContext.Clients.User(n.SenderId.ToString()).SendAsync("NotifyInfo", $"Twoje zaproszenie zostało {(n.Response == InviteStatus.Accepted ? "zaakceptowane" : "odrzucone")}"),
                _hubContext.Clients.User(n.ReciverId.ToString()).SendAsync("NotifyInfo", $"{(n.Response == InviteStatus.Accepted ? "Zaakceptowałeś" : "Odrzuciłeś")} zaproszenie!")
            );
            await Task.WhenAll(
