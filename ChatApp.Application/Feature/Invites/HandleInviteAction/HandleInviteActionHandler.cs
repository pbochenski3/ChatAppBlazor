...
namespace ChatApp.Application.Feature.Invites.HandleInviteAction
{
    public class HandleInviteActionHandler : IRequestHandler<HandleInviteActionCommand, bool>
    {
        ...
        public async Task<bool> Handle(HandleInviteActionCommand r, CancellationToken cancellationToken)
        {
            ...
            {
                var success = await _contactRepo.TryRestoreContactsAsync(invite.SenderID, invite.ReceiverID);
                if (!success)
                {
                    var contacts = await Invite.CreateContact(invite.SenderID, invite.ReceiverID);
                    await _contactRepo.AddContactAsync(contacts[0]);
                    await _contactRepo.AddContactAsync(contacts[1]);
                }
                var chat = await _chatRepo.GetChatAsync(invite.SenderID, invite.ReceiverID);
                if (chat == null)
                {
                    User user1 = await _userRepo.GetByIdAsync(invite.SenderID);
                    User user2 = await _userRepo.GetByIdAsync(invite.ReceiverID);
                    var createdChat = Chat.CreatePrivateChat(user1, user2);
